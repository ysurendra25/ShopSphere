...
function bestKeyword(name) {
  const words = (name || 'product')
    .replace(/[^a-zA-Z0-9 ]/g, ' ')
    .toLowerCase().trim().split(/\s+/);
  return words.find(w => w.length > 3 && !STOP_WORDS.has(w)) || words[0] || 'product';
}
...
function getProductImage(productId, productName, photoUrl) {
  ...
  return `https://loremflickr.com/320/200/${kw}?lock=${productId}`;
}
...
// Helper: returns Authorization header with JWT token from session
  ...
  productsContainer.innerHTML = products.map(product => `
    <div class="product-card admin-product-card">
      <div class="admin-product-img-wrap">
        <img
          src="${getProductImage(product.id, product.name, product.photoUrl)}"
          alt="${escapeHtml(product.name)}"
          class="admin-product-img"
          onerror="this.onerror=null; this.style.display='none'"
        >
        <span class="admin-product-id-badge">ID: ${product.id}</span>
      </div>
